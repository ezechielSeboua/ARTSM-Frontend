...
const ProfessionnelsTab = ({ canEdit = false }) => {
  const [deletingId, setDeletingId] = useState(null);
  const { byRole, loading, fetchAll, deleteUser } = useUserManagementStore();
  const users = byRole.professional;

  useEffect(() => { fetchAll(); }, []);

  const handleDelete = async (id) => {
    if (!window.confirm('Confirmer la suppression de ce professionnel ?')) return;
    setDeletingId(id);
    await deleteUser(id, 'professional');
    setDeletingId(null);
  };

  if (loading) return <LoadingSpinner />;
  if (users.length === 0) return <EmptyState message="Aucun professionnel enregistré." />;
...
export default ProfessionnelsTab;
