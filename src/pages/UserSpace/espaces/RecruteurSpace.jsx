...
import {
  SECTORS, SECTORS_MAP, inputCls, selectCls,
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        ...
        <div className="flex items-center gap-5 mb-6">
          <AvatarEditor
            ...
            bgCls="bg-slate-100"
            spinnerCls="border-slate-900/20 border-t-slate-700"
            textCls="text-slate-700"
            btnCls="bg-slate-700 hover:bg-slate-600"
          />
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="border-t border-slate-100 pt-4">
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-4">Informations de l'entreprise</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Field label="Poste / Fonction">
                  <input type="text" value={profile.job_title} onChange={e => setP('job_title', e.target.value)}
                    className={inputCls} placeholder="DRH, Responsable RH..." />
                </Field>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Secteur">
...
const RecruteurSpace = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-br from-slate-600 to-slate-950 rounded-3xl px-6 pt-6 pb-0 text-white relative overflow-hidden">
          ...
          {TABS.map(tab => (
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-bold whitespace-nowrap rounded-t-xl transition-colors
                ${activeTab === tab.id ? 'bg-slate-50 text-slate-900' : 'text-slate-300 hover:text-white hover:bg-white/10'}`}
            >
